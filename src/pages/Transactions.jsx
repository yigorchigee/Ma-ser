  ...
  return (
    <div className="space-y-10">
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 text-white shadow-2xl">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_10%,rgba(236,72,153,0.2),transparent_40%),radial-gradient(circle_at_80%_0%,rgba(14,165,233,0.16),transparent_38%)]" aria-hidden />
        <div className="p-8 lg:p-10 flex flex-col gap-6 relative">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
            <div className="space-y-3 max-w-3xl">
              ...
              <div className="flex flex-wrap gap-2 text-xs text-white/70">
                <span className="rounded-full bg-white/10 border border-white/15 px-3 py-1">Filter by type</span>
                <span className="rounded-full bg-white/10 border border-white/15 px-3 py-1">Reverse chronological</span>
                <span className="rounded-full bg-white/10 border border-white/15 px-3 py-1">One-click delete</span>
              </div>
            </div>
            <div className="w-full md:w-72 space-y-3 text-right">
              <div className="rounded-full bg-white/15 border border-white/20 px-4 py-2 text-xs font-semibold inline-flex items-center gap-2">
...
function SummaryCard({ label, value, accent }) {
  const colors = {
    indigo: {
      surface: 'from-indigo-50 via-white to-slate-50 border-indigo-100 text-indigo-700',
      dot: 'bg-indigo-500',
    },
    emerald: {
      surface: 'from-emerald-50 via-white to-slate-50 border-emerald-100 text-emerald-700',
      dot: 'bg-emerald-500',
    },
    rose: {
      surface: 'from-rose-50 via-white to-slate-50 border-rose-100 text-rose-700',
      dot: 'bg-rose-500',
    },
  };

  return (
    <div className={`relative overflow-hidden rounded-2xl border bg-gradient-to-br p-5 shadow-md hover:shadow-xl transition-all hover:-translate-y-0.5 ${colors[accent].surface}`}>
      <div className="absolute inset-0 opacity-40 bg-[radial-gradient(circle_at_80%_20%,rgba(255,255,255,0.5),transparent_35%)]" aria-hidden />
      <div className="flex items-center justify-between relative">
        <p className="text-sm font-semibold uppercase tracking-wide text-slate-500">{label}</p>
        <span className={`h-2.5 w-2.5 rounded-full ${colors[accent].dot}`} />
      </div>
      <p className="text-3xl font-black mt-2 flex items-center gap-2 relative">
